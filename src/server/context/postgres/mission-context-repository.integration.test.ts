 ...
 */
describe.skipIf(!dockerAvailable)("PostgresMissionContextRepository (intégration)", () => {
  let ctx: PgContext;
  ...
  describe("spécificités backend", () => {
    ...
    it("persistance effective : snapshots immuables lisibles par version", async () => {
      ...
      expect((await fresh.findVersion("tenant-alpha", "mission-001", 0))?.boundedSummary).toBe(
        "v0",
      );
      expect((await fresh.findVersion("tenant-alpha", "mission-001", 1))?.boundedSummary).toBe(
        "v1",
      );
