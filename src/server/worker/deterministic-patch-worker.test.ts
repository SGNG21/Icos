...
describe("DeterministicPatchWorker", () => {
  ...
  beforeEach(async () => {
    testRoot = await mkdtemp("/tmp/deterministic-patch-worker-");
    taskWorktreeRoot = path.join(testRoot, ".claude", "worktrees");
    repo = path.join(taskWorktreeRoot, TASK_ID);
  ...
  it("accepts only canonical task ids present in the composition catalog", async () => {
    const worker = new DeterministicPatchWorker(policy, new Map(), {
      taskWorktreeRoot,
      runner,
    });
  ...
  it("Mission text cannot influence executable, args, patch, paths, tests, or commit", async () => {
    ...
    const worker = new DeterministicPatchWorker(policy, new Map([[TASK_ID, patch]]), {
      taskWorktreeRoot,
      runner,
    });
  ...
  it("rejects undeclared changes introduced during execution", async () => {
    ...
    const worker = new DeterministicPatchWorker(policy, new Map([[TASK_ID, definition()]]), {
      taskWorktreeRoot,
  ...
  it("rejects the active source worktree even when it is an absolute clean repository", async () => {
    const worker = new DeterministicPatchWorker(policy, new Map([[TASK_ID, definition()]]), {
      taskWorktreeRoot,
      runner,
    });

    const workerId = await worker.spawn(input(testRoot));
    const result = (await worker.waitForCompletion(workerId)) as DeterministicPatchWorkerResult;

    expect(result.outcome).toBe("FAILED");
    expect(result.errorCode).toBe("INVALID_WORKTREE");
    expect(await readFile(path.join(repo, "source.txt"), "utf8")).toBe(ORIGINAL);
    expect(runner.calls).toEqual([]);
  });

  it("rejects undeclared changes introduced by a focused test", async () => {
    const testMutatingRunner: DeterministicPatchProcessRunner = {
      run: async (executable, args, options) => {
        if (executable === "pnpm") {
          await writeFile(path.join(repo, "test-side-effect.txt"), "undeclared\n");
        }
        return runner.run(executable, args, options);
      },
    };
