...
async function captureSourceState(repoRoot: string) {
  const trackedPaths = (await git(repoRoot, ["ls-files", "-z"]))
    .split("\0")
    .filter(Boolean);
...
function assertNoDangerousEffects(
  ...
  for (const evidence of Object.values(taskEvidence ?? {})) {
    for (const process of (evidence.workerResult as DeterministicPatchWorkerResult | undefined)?.evidence?.processes ?? []) {
...
async function assertTerminalFailure(
  ...
  const persistedDag = result.dag ? await fixture.supervisorRepository.findDagById(result.dag.id) : null;
  expect(persistedDag?.status).toBe("FAILED");
  expect(persistedDag?.status).not.toBe("COMPLETED");
  expect(Object.values(persistedDag?.nodes ?? {}).every((node) => node.status === "SUCCEEDED")).toBe(true);
...
describe("planAndExecuteMission runtime safety", () => {
  it("composes one successful real mission execution", async () => {
    ...
    try {
      ...
      expect(Object.values(result.executionResult.dag.nodes).every((node) => node.status === "SUCCEEDED")).toBe(true);
      ...
      expect(workerResult.evidence.processes.some((process) => process.executable === "pnpm" && process.args.join(" ").includes(FOCUSED_TEST_PATH))).toBe(true);
      expect(workerResult.evidence.processes.some((process) => process.args.join(" ").includes("rm -rf") || process.args.join(" ").includes("touch pwned") || process.args.join(" ").includes("deploy production"))).toBe(false);
      ...
      expect(await git(tempRepoRoot, ["branch", "--list", "integration/*"])).toContain("integration/");
    } finally {
      process.chdir(previousCwd);
      await rm(tempRepoRoot, { recursive: true, force: true });
    }
  ...
  it("fails closed on a real integration conflict and aborts cherry-pick", async () => {
    ...
    try {
      ...
      expect(fixture.worktrees.integrationStatusesBeforeCleanup[0]).toContain(
        `AA ${PROOF_PATH}`,
      );
    } finally {
      await fixture.dispose();
