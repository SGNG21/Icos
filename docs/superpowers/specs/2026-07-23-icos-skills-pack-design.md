...
- **`icos-security` vs `icos-postgresql`** — `icos-security` statue sur _qui_
  peut faire _quoi_ (rôles, permissions, approbation humaine, contenu
  d'audit). `icos-postgresql` statue sur _comment_ la donnée est stockée et
...
- **`icos-agent-orchestration` vs `icos-workflows-temporal`** —
  `icos-agent-orchestration` définit les règles _métier_ : quels agents
  ...
  _substrat d'exécution_ : comment un processus long, interruptible et
...
- **`icos-agent-orchestration` vs `icos-mcp-integrations`** —
  `icos-agent-orchestration` gouverne le modèle _interne_ d'agents ICOS (leur
  cycle de vie, leurs décisions). `icos-mcp-integrations` gouverne
  l'_exposition/consommation_ de capacités via le protocole MCP, qu'il
...
- **`icos-legacy-reuse` vs toutes les autres** — `icos-legacy-reuse` ne
  ...
  _méthode_ : elle s'invoque avant qu'une des 7 autres compétences ne
