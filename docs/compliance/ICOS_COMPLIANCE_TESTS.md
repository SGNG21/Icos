...
| CT-DOC-03 | Aucune donnée de catégorie spéciale (art. 9 RGPD) n'est introduite. | `01-classification.md` §4 |
...
| ID | Scenario | Compliance intent | Expected invariant | Target lot | Tests associés (CT-DOC / CT-AUTO) | Current status |
|---|---|---|---|---|---|---|
| 001 | tenant isolation | Un tenant ne peut pas accéder aux données d'un autre tenant. | Isolation multi-tenant | COMPLIANCE-1 | CT-AUTO-09, CT-AUTO-10 | DOCUMENTED_ONLY |
...
| 008 | external transfer audit | Tout transfert de données C3 vers un provider externe est audité. | Auditabilité des transferts externes | G1 | CT-AUTO-06 | DOCUMENTED_ONLY |
| 009 | sensitive action cannot bypass approval | Une action marquée sensible ne peut pas contourner le Policy Engine. | PolicyEngine est incontournable pour actions sensibles | D1 | — (tests D1 à définir) | DOCUMENTED_ONLY |
| 010 | app DB role cannot TRUNCATE audit | Le rôle applicatif PostgreSQL ne peut pas TRUNCATE la table d'audit. | Audit append-only garanti par PostgreSQL | COMPLIANCE-1 | — (tests COMPLIANCE-1 à définir) | DOCUMENTED_ONLY |
| 011 | cross-tenant IDOR denial | Un identifiant d'un autre tenant ne peut pas être utilisé pour accéder à des données. | Frontière d'isolation Tenant respectée | COMPLIANCE-1 | CT-AUTO-09, CT-AUTO-10 | DOCUMENTED_ONLY |
| 012 | provider without approved compliance profile denied | Un provider sans `ProviderComplianceProfile` approuvé est refusé pour toute donnée C2+. | ProviderComplianceProfile requis pour données C2+ | D3 | — (tests D3 à définir) | DOCUMENTED_ONLY |
| 013 | memory retention requires explicit policy | La mémoire agent ne peut retenir une donnée sans politique de rétention associée. | RetentionPolicy requise pour MemoryPort | E1 | CT-DOC-04 | DOCUMENTED_ONLY |
| 014 | deleted source invalidates derived memory | Une mémoire dérivée est invalidée si sa source est supprimée. | DataProvenance respectée dans MemoryPort | E1 | — (tests E1 à définir) | DOCUMENTED_ONLY |
| 015 | observability payload redaction | Les payloads C3 sont automatiquement masqués dans les traces d'observabilité. | Pas de C3 dans l'observabilité technique | COMPLIANCE-1 | CT-DOC-06, CT-AUTO-08 | DOCUMENTED_ONLY |
| 016 | purpose mismatch denied | Un traitement sollicité ne correspondant à aucune finalité déclarée est refusé. | ProcessingPurpose vérifié par PolicyEngine | D1 | CT-DOC-02 | DOCUMENTED_ONLY |
| 017 | legal hold prevents destructive deletion | Une donnée sous gel (legal hold) ne peut pas être détruite par la purge automatique. | LegalHold suspend RetentionPolicy | COMPLIANCE-2 | — (tests COMPLIANCE-2 à définir) | DOCUMENTED_ONLY |
| 018 | connector SEND requires external-effect policy | Un connecteur externe nécessite une policy de type `external_effect` pour envoyer des données. | Effet externe nécessite policy dédiée | G1 | CT-DOC-06 | DOCUMENTED_ONLY |
| 019 | provider trainingAllowed enforcement | Les données C3 ne sont jamais envoyées à un provider dont `trainingAllowed` est vrai sans consentement explicite. | ProviderComplianceProfile.trainingAllowed respecté | D3 | — (tests D3 à définir) | DOCUMENTED_ONLY |
| 020 | tenant-scoped export contains no foreign data | L'export des données d'un tenant ne contient aucune donnée d'un autre tenant. | Frontière Tenant respectée à l'export | COMPLIANCE-3 | — (tests COMPLIANCE-3 à définir) | DOCUMENTED_ONLY |
| 021 | consent record | L'enregistrement, la preuve, le retrait et l'effet du retrait du consentement sont tracés et opposables. | ConsentRecord horodaté, append-only, révocable | COMPLIANCE-2 | — (tests COMPLIANCE-2 à définir) | DOCUMENTED_ONLY |
| 022 | data portability | L'export JSON des données d'un tenant respecte les frontières et ne contient aucune donnée étrangère. | Tenant boundaries respectées à l'export | COMPLIANCE-3 | — (tests COMPLIANCE-3 à définir) | DOCUMENTED_ONLY |
| 023 | personal data breach | La détection, qualification, escalade et notification CNIL (72h) d'une violation de données. | Workflow breach actionnable dans les délais | COMPLIANCE-2 | — (tests COMPLIANCE-2 à définir) | DOCUMENTED_ONLY |
| 024 | right to erasure | La demande d'effacement est vérifiée, exécutée (soft-delete, purge), propagée aux données dérivées et suspendue si legal hold. | DeletionRequest → vérification → soft-delete → purge | COMPLIANCE-2 | — (tests COMPLIANCE-2 à définir) | DOCUMENTED_ONLY |
